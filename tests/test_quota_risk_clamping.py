...
def test_tight_stop_scales_down_to_fit_60_quota():
    """
    Verifies that when remaining quota is limited (e.g. ₹60.00 after ₹141 loss),
    a setup with a valid structural stop scales down its position size so that
    total risk is strictly <= ₹60.00.
    """
    ...
    # Risk must be strictly clamped to <= 60.00 INR
    assert res.risk_amount <= 60.01
...
def test_wide_stop_rejected_when_exceeding_quota():
    ...
    # Even 0.1 SOL * $10 * 87.5 = ₹87.50, which exceeds remaining ₹60 quota
    res = PositionSizer.calculate_position(
        entry=105.0,
        stop_loss=115.0,
        account_equity=4200.0,
